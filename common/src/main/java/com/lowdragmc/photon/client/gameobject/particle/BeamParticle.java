...
public abstract class BeamParticle extends LParticle {
    ...
    public void renderInternal(@Nonnull Matrix4f matrix, @Nonnull VertexConsumer pBuffer, @Nonnull Camera camera, float partialTicks) {
        ...
        pBuffer.vertex(matrix, p1.x, p1.y, p1.z).uv(u0, v0).color(r, g, b, a).uv2(light).endVertex();
        pBuffer.vertex(matrix, p0.x, p0.y, p0.z).uv(u0, v1).color(r, g, b, a).uv2(light).endVertex();
        pBuffer.vertex(matrix, p4.x, p4.y, p4.z).uv(u1, v1).color(r, g, b, a).uv2(light).endVertex();
        pBuffer.vertex(matrix, p3.x, p3.y, p3.z).uv(u1, v0).color(r, g, b, a).uv2(light).endVertex();
    }
