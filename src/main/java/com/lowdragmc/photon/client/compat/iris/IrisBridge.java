 ...
 * {@code PhotonWorldRenderState}) only survives because HotSpot resolves constant-pool entries lazily,
...
public interface IrisBridge {
     ...
     */
    /**
     * A view over the pack's resolved colour attachment, for the composite to draw into. Null when the
     * layout offers nothing writable. 26.1 composites through a {@code RenderPipeline}, so this is a
     * texture view rather than 1.21's owned framebuffer id.
     */
    default @Nullable GpuTextureView compositeTarget(IrisFrameTarget target) {
        return null;
    }
