...
public class EditorModelInstanceScenario implements UIScenario {
    ...
    private static final Identifier MODEL = Photon.id("models/uitest_editor_model.glb");
    ...
    public void define(ScenarioBuilder s) {
        ...
        // not exotic — any project carrying a stale reference has one. On 1.21 it handed back a VANILLA
        // program with no per-instance attributes, so the CPU path drew correctly and instancing drew
        // every model untransformed at the origin. 26.1 can only get this wrong a different way: a
        // RenderType with no InstancedRecipe makes bakeInstancedGroup bail to the CPU path, so the two
        // would disagree in framerate rather than in geometry. Either way the check is the same — the
        // missing material must look identical with instancing off and on.
    ...
    private static void aimSceneCamera(TestContext ctx) {
        // 26.1: Scene.getRenderer() is generic (<T> T), so the witness is not optional here
        var renderer = editor(ctx).sceneView.sceneEditor.scene.<WorldSceneRenderer>getRenderer();
    ...
    private static ParticleEmitter emitter(boolean perParticle, boolean instanced) {
        ...
        renderer.getMaterials().add(new MaterialSetting(new TextureMaterial(
                Identifier.parse("textures/block/white_concrete.png")))
                .setDepthMask(true).setCull(true));
        return emitter;
    }
