...
public class ObjectSpaceScenario implements UIScenario {
    ...
    /** Every render-path variant — each compiles its own branch of the new matrix builders. */
    ...
    public void define(ScenarioBuilder s) {
        s.step("every particle.glsl variant still compiles with the object<->world matrices", ctx -> {
            var material = new TextureMaterial();
            VARIANTS.forEach((name, context) -> {
                // getShader compiles through LDShaderInstance and rethrows a failed compile, so an invalid
                // variant fails here rather than silently rendering wrong in-game.
