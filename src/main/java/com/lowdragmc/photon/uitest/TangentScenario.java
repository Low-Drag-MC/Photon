 ...
 *
 * <p><b>The GLSL compiles.</b> {@code particle.glsl} is a seven-way {@code #ifdef} chain and the tangent
 * added an eighth branch ({@code PHOTON_TANGENT}) that repacks the model path's attribute layout. Only a
 * real driver can say whether each branch is valid GLSL, and nothing else in the build ever compiles them
 * — a broken variant would surface as an in-game render failure on whichever emitter happened to use it.
 *
...
public class TangentScenario implements UIScenario {
    ...
    /** Every render-path variant, plus the tangent-enabled model path. */
    private static final Map<String, MaterialContext> VARIANTS = Map.of(
            ...
            "PARTICLE_MODEL_INSTANCE+PHOTON_VAT+PHOTON_TANGENT", MaterialContext.PARTICLE_MODEL_INSTANCE_VAT_TANGENT,
            "TRAIL_INSTANCE", MaterialContext.TRAIL_INSTANCE,
            "ARA_TRAIL_INSTANCE", MaterialContext.ARA_TRAIL_INSTANCE,
            "ARA_TRAIL_TUBE_INSTANCE", MaterialContext.ARA_TRAIL_TUBE_INSTANCE,
            "BEAM_INSTANCE", MaterialContext.BEAM_INSTANCE);

    @Override
    public void define(ScenarioBuilder s) {
        s.step("every particle.glsl variant compiles on the driver", ctx -> {
            var material = new TextureMaterial();
            VARIANTS.forEach((name, context) -> {
                // getShader compiles through LDShaderInstance and rethrows a failed compile, so an
                // invalid variant fails the check here rather than silently rendering wrong in-game.
                ...
                ctx.check("%s compiles".formatted(name), failure == null && shader != null,
